...
What's cool about version control systems like [Git][6]? They're *immutable*! Every state change made to a project through Git preserves the previous state in history. Why is this cool? For starters it allows a distributed team to work on the same project without messing up each other's work. Unless [rebases][7] on shared branches commonly occur in your team, the immutable Git history means that you can check out a commit and have absolute certainty that nobody can change that state of the project from under your feet.
...
Some languages, like [Haskell][8], enforce immutability, while others do no such thing. In the case of JavaScript, immutability can indeed be a very hard thing to strive for. Because the language does not enforce immutability, it's a constraint you'll have to put on yourself.
